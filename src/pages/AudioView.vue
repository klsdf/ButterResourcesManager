...
export default {
  ...
  methods: {
    ...
    isDisguiseModeEnabled() {
      try {
        ...
        if (settings) {
          ...
          console.log('AudioView: 检查伪装模式设置:', isEnabled, '设置数据:', parsedSettings.disguiseMode)
          return isEnabled
        }
        console.log('AudioView: 没有找到设置数据，伪装模式默认关闭')
        return false
      } catch (error) {
        console.error('AudioView: 检查伪装模式设置失败:', error)
        return false
      }
    },
    ...
    async loadDisguiseImage(imagePath) {
      console.log('AudioView: 开始加载伪装图片，原始路径:', imagePath)
      try {
        ...
        console.log('AudioView: 获取到伪装图片路径:', disguiseImage)
        // 使用Vue的响应式更新
        this.$set ? this.$set(this.disguiseImageCache, imagePath, disguiseImage) : (this.disguiseImageCache[imagePath] = disguiseImage)
        // 强制更新组件
        this.$forceUpdate()
        console.log('AudioView: 伪装图片已更新到缓存')
      } catch (error) {
        console.error('AudioView: 加载伪装图片失败:', error)
      }
    },
    ...
    getDisplayName(audio) {
      if (this.isDisguiseModeEnabled()) {
        // 检查伪装文字缓存
        if (this.disguiseTextCache[audio.id]) {
          return this.disguiseTextCache[audio.id]
        }
        
        // 异步获取伪装文字
        this.loadDisguiseText(audio.id)
        return audio.name // 先返回原始名称，等异步加载完成
      }
      return audio.name
    },
